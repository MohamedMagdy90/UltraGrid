...
build:
git clean -xdf && make clean && docker build -t boostnova/ultraboost${TAG} -f Dockerfile .
uninstall:
conda env remove -n ultraboost -y
install:
@mkdir -p logs
@echo "Using env file: $(ENV_FILE)"
@if conda env list | awk '{print $$1}' | grep -qx ultraboost; then \
conda env update -n ultraboost -f "$(ENV_FILE)"; \
else \
conda env create -n ultraboost -f "$(ENV_FILE)"; \
fi
@if [ "$$(uname)" = "Darwin" ]; then \
conda install -n ultraboost -y appnope; \
fi
@conda run -n ultraboost conda develop .
@conda run -n ultraboost python -m pip install --no-deps -r setup/pip_packages.txt > logs/pip_install.log 2>&1
@conda run -n ultraboost pre-commit install
...
@conda run -n ultraboost --no-capture-output python setup.py build_ext --inplace
run:
conda run -n ultraboost --no-capture-output ./bin/ultraboost_quickstart.py $(ARGS)
setup:
