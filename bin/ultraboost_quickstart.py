...
from bin.ultraboost import UIStartListener, detect_available_port
...
def autofix_permissions(user_group_spec: str):
    ...
    gateway_path: str = Path.home().joinpath(".ultraboost-gateway").as_posix()
    subprocess.run(
        f"cd '{project_home}' && "
...
async def quick_start(args: argparse.Namespace, secrets_manager: BaseSecretsManager):
    """Start UltraBoost using unified HummingbotApplication in either UI or headless mode."""
    ...
    init_logging("ultraboost_logs.yml", client_config_map)
...
async def run_application(hb: HummingbotApplication, args: argparse.Namespace, client_config_map):
    ...
    if args.headless:
        ...
        from hummingbot import init_logging
        log_file_name = hb.strategy_file_name.split(".")[0] if hb.strategy_file_name else "ultraboost"
        init_logging("ultraboost_logs.yml", hb.client_config_map,
